...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //take intent from catalog activity
...
//
//        if (newRowId==-1){
//            Toast.makeText(this, "Error in Saving Text", Toast.LENGTH_SHORT).show();
//        }else {
//            Toast.makeText(this, "Values successfully saved"+newRowId, Toast.LENGTH_SHORT).show();
//        }
        if (mCurrentPetUri==null){
            //This is a new pet,so insert a new pet into the provider
            // returning the content URI for the new part
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        //if there is data on cursor
        else if (cursor.moveToNext()){
            //find the columns of pet attribute
            int nameColumnIndex=cursor.getColumnIndex(PetEntry.COLUMN_PET_NAME);
            int breedColumnIndex=cursor.getColumnIndex(PetEntry.COLUMN_PET_BREED);
            int genderColumnIndex=cursor.getColumnIndex(PetEntry.COLUMN_PET_GENDER);
            int weightColumnIndex=cursor.getColumnIndex(PetEntry.COLUMN_PET_WEIGHT);

            //Extract values of given column
            String name=cursor.getString(nameColumnIndex);
            String breed=cursor.getString(breedColumnIndex);
            Integer gender=cursor.getInt(genderColumnIndex);
            Integer weight=cursor.getInt(weightColumnIndex);
